...
python3 -m crossbeam.experiment.run_baseline_synthesizer_handwritten_tasks \
  ...
  --timeout=${timeout} \
  --num_process=${num_process} \
  --json_results_file="comparisons/synthetic_baseline_enumeration_timeout${timeout}.json" \
...
for seed in 0 1 2 3 4; do
  python3 -m crossbeam.experiment.run_baseline_synthesizer_handwritten_tasks \
    ...
    --timeout=${timeout} \
    --num_process=${num_process} \
    --json_results_file="comparisons/synthetic_baseline_enumeration_timeout${timeout}_shuffleops.run${seed}.json" \
    --synthetic_test_tasks=True \
    --shuffle_ops=True \
    --seed=${seed} \
    --verbose
