...
def perform_search(domain, min_weight, max_weight, num_examples, num_inputs,
    ...
    dummy_outputs = ['~', '&', '=', '^', '*', ",", "s", "a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m",
                     "n"][:num_examples]
...
def dynamic_task_gen(args, domain, dreamcoder_train_tasks=None, split="train"):
    ...
    print("Forced timouter will happen after ", args.data_gen_timeout * math.ceil(
        args.num_searches / args.num_datagen_proc) + 120 + args.dynamic_time_increase * (
                      len(domain.operations) - args.num_starting_ops), " seconds")
    signal.alarm(args.data_gen_timeout * math.ceil(
        args.num_searches / args.num_datagen_proc) + 120 + args.dynamic_time_increase * (
                             len(domain.operations) - args.num_starting_ops))
